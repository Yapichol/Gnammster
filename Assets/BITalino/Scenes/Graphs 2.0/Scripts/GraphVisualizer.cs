...
public class GraphVisualizer : MonoBehaviour {
	...
	[SerializeField] private float m_heartRate;
	...
	void Awake () {
		...
		m_fs = BitalinoManager.SamplingFrequency;
		...
		for (int k = 0; k < (nDimensions > 1 ? nPointsPerDimension : 1); k++) {
			...
			for (int j = 0; j < (nDimensions > 2 ? nPointsPerDimension : 1); j++) {
				...
				for (int i = 0; i < (nDimensions > 0 ? nPointsPerDimension : 1); i++) {
					...
					m_points [index].color = new Color (0f, scale - p.z, scale - p.z, alpha);
					m_points [index++].size = pointSize;
				}
			}
		}
